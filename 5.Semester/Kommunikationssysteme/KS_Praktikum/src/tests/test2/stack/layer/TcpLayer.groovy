...
import tests.test2.stack.fsm.Event
import tests.test2.stack.fsm.State
import tests.test2.stack.layer.idu.*
import tests.test2.stack.layer.pdu.T_PDU
...
class TcpLayer {
    ...
    /** TCP-Windowsize */
    static final int WINDOWSIZE = 512
            ...
            // Daten empfangen
                ...
                case (State.S_RCVD_DATA):
                    ...
                    if (recvSeqNum == sendAckNum) {
                        ...
                        sendSynFlag = false
                        sendAckFlag = true
                        sendAckNum = sendAckNum + recvData.size()
                        sendFinFlag = false
                        sendRstFlag = false
                        sendData = ""
    ...
    //------------------------------------------------------------------------------
    /**
     * Liefert die Message-Queue in den Sender
     * @return fromAppQ
     */
    ...
    //------------------------------------------------------------------------------
    /**
     * Liefert die Message-Queue in den Empfaenger
     * @return fromIpQ
     */
    ...
    void start(MQueue<TA_IDU> toAppQ, MQueue<TRI_IDU> toIpQ, ConfigObject config) {
        ...
        // Parameter: ala Thread auszuführende Closure, initiale Verzögerung, periodische Verzögerung
