...
class Utils {
    ...
    /**
     ...
     <p/>
     Verwendung:<br>
     writeLog("Klasse A", "Methode B", "Hier", 3) // normale Meldung<br>
     writeLog("Kl A", "Meth C", "var X: ${varX}", 3) // Debug-Meldung<br>
    ...
    public static void writeLog(String klass, String method, String comment, int dbgLvl) {
        ...
        if (dbgLvl in debugLevel) {
            ...
            // u.U. Verzögern
            if (debugDelay) sleep(debugDelay)
        }
    }
    ...
    static ConfigObject getConfig(String test, String host) {
        ...
        if (System.getProperty("stand.alone"))
            // Ja
            path = new File("../../../../../src/tests/${test}/config").text // Konfigurationsdati lesen
        else
            // Nein
            path = new File("src/tests/${test}/config").text // Konfigurationsdati lesen
        ...
        debugLevel = co1.debugLevel
        debugDelay = co1.debugDelay
